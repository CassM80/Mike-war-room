...
(() => {
  ...
  function teamLimit(team,p){
    ...
    let mult=pers.aggression*positionNeed(team,p.pos)*(pers[p.pos.toLowerCase()]||1);
    if(pers.chaos)mult*=.75+Math.random()*.65;
    mult*=.92+Math.random()*.18;
    if(p.pos==="K"||p.pos==="DEF")mult=Math.min(mult,1.05);
    return Math.max(1,Math.min(legalMax(team),Math.round(market*mult)));
  }
  ...
  function primeAIBidding(){
    ...
    const leader=bidders[0],market=playerMarket(p),pause=Math.min(leader.limit,Math.max(n.currentBid,Math.round(market*(.62+Math.random()*.18))));
  ...
  function renderMock(){
    ...
    if(mock?.nomination){
      ...
      q("mockCurrentBid").textContent=`$${n.currentBid}`;q("mockHighBidder").textContent=teamName(n.highBidder);q("mockMarket").textContent=`$${market}`;q("mockSafeMax").textContent=`$${safe}`;
