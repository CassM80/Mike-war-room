...
function positionPool(pos){
  ...
  return PLAYERS.filter(p=>p.pos===pos&&p.active===true&&CURRENT_NFL_TEAMS.has(String(p.team||"").toUpperCase())&&(marketSeedValue(p)>0||marketRankFor(p)>0)&&(marketRankFor(p)<=limits[pos]||marketSeedValue(p)>0)).sort((a,b)=>marketSeedValue(b)-marketSeedValue(a)||(marketRankFor(a)||9999)-(marketRankFor(b)||9999)||a.name.localeCompare(b.name));
}
...
function rebuildOverallDNARanks(){
 ...
 auto.sort((a,b)=>Number(b.ev.value||0)-Number(a.ev.value||0)||(marketRankFor(a.p)||9999)-(marketRankFor(b.p)||9999)||a.p.name.localeCompare(b.p.name));
...
function positionAdjustment(pos,p,profile,m){let adj=0;adj+=profile.investment==='HIGH'?4:profile.investment==='LOW'?-4:0;adj+=({LOVE:10,LIKE:5,NEUTRAL:0,FADE:-6,OUT:-15}[m.gut[p.name]]||0);adj+=({SMASH:8,BUY:4,FAIR:0,HIGH:-5,PASS:-12}[m.prices[p.name]]||0);Object.entries(m.pairs).forEach(([key,winner])=>{if(winner===p.name)adj+=4;else if(key.split('|').includes(p.name))adj-=2;});const a=marketRankFor(p)||999;if(profile.risk==='UPSIDE'&&a>45&&a<180)adj+=3;if(profile.risk==='FLOOR'&&a<=70)adj+=3;const leagueDelta=(Number(profile.leagueIntensity||1)-1)*3;adj+=Math.max(-3,Math.min(4,leagueDelta));return adj;}

function positionCandidates(pos){const profile=computePositionProfile(pos),m=dnaModule(pos);return positionPool(pos).map(p=>({p,score:marketSeedValue(p)*2+(250-Math.min(250,marketRankFor(p)||250))*.06+positionAdjustment(pos,p,profile,m)*2})).sort((a,b)=>b.score-a.score).map(x=>x.p);}
...
function buildPositionBoard(pos){activatePersonalization();const m=dnaModule(pos),profile=computePositionProfile(pos),players=positionCandidates(pos),ctx=leagueDNAContext(),positionBudgets=suggestedPositionBudgets(),limit=pos==='QB'||pos==='TE'?36:90;let changed=0;players.slice(0,limit).forEach((p,i)=>{const old=getPersonalEvaluation(p.name);if(old&&!String(old.notes||'').startsWith('Position DNA:'))return;const reaction=m.gut[p.name],price=m.prices[p.name],avoid=reaction==='OUT'||price==='PASS';const tier=i<(pos==='QB'||pos==='TE'?4:8)?'1':i<(pos==='QB'||pos==='TE'?12:24)?'2':i<(pos==='QB'||pos==='TE'?24:48)?'3':'4';const conviction=avoid?1:(reaction==='LOVE'&&price==='SMASH'?5:reaction==='LOVE'||reaction==='LIKE'?4:reaction==='FADE'?2:3);const draftEv={name:p.name,conviction,rank:i+1,tier,favorite:!avoid&&(reaction==='LOVE'||i<3),flagPlant:!avoid&&(reaction==='LOVE'&&price==='SMASH'),sleeper:!avoid&&i>12&&(reaction==='LIKE'||profile.risk==='UPSIDE'),avoid};const pricing=restrainedPersonalPricing(p,draftEv);personalEvaluations[playerKey(p.name)]={...draftEv,value:pricing.value,hardStop:pricing.hardStop,valueSource:'auto',hardStopSource:'auto',valuationModelVersion:3,notes:`Position DNA: ${pos} • ${profile.leagueDemand.toLowerCase()} • ${reaction||'profile fit'}${price?' • '+price.toLowerCase():''} • ${ctx.teams}T ${ctx.scoring} ${money(ctx.budget)}`,updatedAt:new Date().toISOString()};changed++;});m.complete=true;m.profile={...profile,suggestedBudget:positionBudgets[pos],leagueFingerprint:ctx.fingerprint};savePositionDNA();rebuildOverallDNARanks();savePersonalEvaluations(true);renderPersonalBoard();renderBulkBoard();renderCore();return changed;}
